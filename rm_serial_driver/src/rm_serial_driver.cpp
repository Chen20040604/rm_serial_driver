...
RMSerialDriver::RMSerialDriver(const rclcpp::NodeOptions& options)
  ...
  serial_pub_ = this->create_publisher<auto_aim_interfaces::msg::ReceiveSerial>(
      "/angle/init", 10);
    ...
    try {
      ...
      if (receiving_data) {
        ...
        if (header[0] == 0xAA) {
          ...
          //     std::cout << "data_buffer[" << i << "]:" << data_buffer[i] <<
          //     std::endl;
          //     //std::cout << "data_buffer[" << i << "]:" << std::hex
          //     <<std::uppercase <<a <<std::endl;
          // }
          // 处理接收到的数据
...
void RMSerialDriver::navsetData(const geometry_msgs::msg::Twist& cmd_vel) {
  ...
  try {
    packet.header = 0xA5;
    packet.naving = 1;
    packet.nav_x = cmd_vel.linear.y * 4000;
    packet.nav_y = -cmd_vel.linear.x * 4000;
